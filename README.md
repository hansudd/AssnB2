...
snapshot of one’s data. Specifically, the function in this package
provides a data snapshot of two numeric variables, by returning summary
statistics for the numeric variables (mean, median, min and max), as
well as a visualization (scatterplot) and the adjusted R squared value
to provide a sense of the linear relationship between the variables (if
any). Generally, the adjusted r squared value, a value between 0 and 1
(displayed as a percentage by the function), represents the proportion
of variance observed for a dependent variable explained by the
